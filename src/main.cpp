...
int main(int argc, char **argv) {
  ...
  std::static_pointer_cast<Sphere>(s3)->set_radius(1.f);
  std::static_pointer_cast<Sphere>(s3)->set_center({1.1f, 0.5f, 0.f, 1.f});
  ...
  gridBounds.extend_by(std::static_pointer_cast<Sphere>(s3)->bounding_box().bounds[0]);
  gridBounds.extend_by(std::static_pointer_cast<Sphere>(s3)->bounding_box().bounds[1]);
  ...
  KDtreeMidpoint *tree = new KDtreeMidpoint();
  tree->construct(gridBounds, objs, 2, i);
